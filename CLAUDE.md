...
- **Past nights archive** (`NightsArchivePage.jsx`, page id `nights`): its own admin-only page, reached only via a minimal text link at the bottom of the admin board (deliberately not a data-bearing panel — it costs nothing until clicked) rather than a section on the board itself. On mount it checks `GET /api/login` and bounces to `admin` if not authenticated, then lists every closed night (`GET /api/nights`) newest-first — date, picked-up/cancelled counts, total. Clicking a night expands it in place to show the archived per-order detail (`formatOrderItems()` in `utils/orders.js`, shared with the admin Finished list) — no separate route or extra fetch, since the detail already came down with the list.

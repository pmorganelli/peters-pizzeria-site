...
- **Customer** (`OrderPage.jsx`): cart built from `MENU_DATA` (qty steppers), persisted to localStorage (`pp_cart2`, `pp_who`, `pp_order_id`; the legacy `pp_cart` {name: qty} shape is migrated on load); on submit → POST `/api/orders`; confirmation screen shows a pickup code and polls order status every 8s. Payment stays Venmo/Zelle at pickup — no payment processing.
...
- **Rate limits** (`rateLimit()` in `api/_lib/store.js`, fixed-window on Redis INCR): orders 15/IP/10min (campus NAT puts dorms behind one IP) + 120 global/10min, login 8/IP/5min. Friendly 429 messages.
